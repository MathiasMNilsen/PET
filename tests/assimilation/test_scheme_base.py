...
def test_runs_prior_forecast_before_iterating(in_tmp_dir):
    ens = FakeEnsemble()
    DecreasingMisfitScheme(ens, maxiter=1, logit=False).assimilation_loop()
...
def test_subclass_convergence_hook(in_tmp_dir):
    ...
    res = StopsAfterTwo(FakeEnsemble(), maxiter=50, logit=False).assimilation_loop()
    assert res.success is True
    assert res.nit == 2
...
def test_result_is_attribute_accessible(in_tmp_dir):
    res = DecreasingMisfitScheme(FakeEnsemble(), maxiter=2, logit=False).assimilation_loop()
...
def test_assimilate_classmethod_matches_manual_run(in_tmp_dir):
    ...
    manual = DecreasingMisfitScheme(FakeEnsemble(), maxiter=3, logit=False).assimilation_loop()
