...
Kept separate from :mod:`pipt.update_schemes.analysis.base`: the concrete
flavours live in ``update_methods_ns`` and import ``analysis.base``, so
importing them from the base module -- or from this package's ``__init__`` --
would form a cycle. ``tests/test_import_hygiene.py`` guards that.
"""
